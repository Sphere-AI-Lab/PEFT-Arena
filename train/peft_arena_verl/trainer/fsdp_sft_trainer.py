...
from peft import AdaLoraConfig, IA3Config, LoraConfig, MissConfig, OFTConfig, TaskType, VeraConfig, get_peft_model
...
from peft_arena_verl.utils.checkpoint_metadata import save_checkpoint_metadata
from peft_arena_verl.utils.dataset import SFTDataset
from peft_arena_verl.utils.keeplora import (
    freeze_keeplora_down_projection,
    initialize_keeplora_adapters,
    resolve_keeplora_principal_rank,
)
...
def _get_lora_rank(model_config):
    lora_rank = _cfg_get(model_config, "lora_rank", 0)
    if lora_rank <= 0:
        lora_rank = _cfg_get(model_config, "vera_rank", 0)
    return lora_rank


def _get_lora_variant(model_config):
    variant = _cfg_get(model_config, "lora_variant", None)
    if variant:
        return str(variant).lower()
    if _cfg_get(model_config, "use_adalora", False):
        return "adalora"
    if _cfg_get(model_config, "use_loraplus", False):
        return "loraplus"
    if _cfg_get(model_config, "use_ia3", False):
        return "ia3"
    return None


def _is_keeplora_variant(model_config) -> bool:
    return _get_lora_variant(model_config) == "keeplora"


def _build_lora_config(model_config, total_training_steps=None):
    lora_rank = _get_lora_rank(model_config)
    variant = _get_lora_variant(model_config)
    target_modules = convert_to_regular_types(_cfg_get(model_config, "target_modules", None))
    target_parameters = convert_to_regular_types(_cfg_get(model_config, "target_parameters", None))
    exclude_modules = convert_to_regular_types(_cfg_get(model_config, "exclude_modules", None))
    lora_dropout = _cfg_get(model_config, "lora_dropout", 0.05)

    if variant == "miss":
        return MissConfig(
            task_type=TaskType.CAUSAL_LM,
            r=lora_rank,
            miss_dropout=lora_dropout,
            target_modules=target_modules,
            bias="none",
        )
    if variant == "adalora":
        total_step = _cfg_get(model_config, "adalora_total_steps", 0)
        if total_step <= 0 and total_training_steps is not None:
            total_step = total_training_steps
        return AdaLoraConfig(
            task_type=TaskType.CAUSAL_LM,
            r=lora_rank,
            lora_alpha=_cfg_get(model_config, "lora_alpha", 16),
    ...
    return LoraConfig(
        ...
        lora_alpha=_cfg_get(model_config, "lora_alpha", 16),
        lora_dropout=lora_dropout,
        use_qalora=_cfg_get(model_config, "use_qalora", False),
        use_rslora=_cfg_get(model_config, "use_rslora", False),
        use_dora=_cfg_get(model_config, "use_dora", False),
        init_lora_weights=_cfg_get(model_config, "init_lora_weights", True),
        target_modules=target_modules,
...
def _build_oft_config(model_config):
    print('model_config', model_config)
    normalize_rot = _cfg_get(model_config, "oft_normalize_rotation", "none")
    oft_block_size = _cfg_get(model_config, "oft_block_size", 0)
    target_modules = convert_to_regular_types(_cfg_get(model_config, "target_modules", None))
    exclude_modules = convert_to_regular_types(_cfg_get(model_config, "exclude_modules", None))
    
    if normalize_rot != "none":
        from peft import OFTV3Config
        return OFTV3Config(
            task_type=TaskType.CAUSAL_LM,
            oft_block_size=oft_block_size,
            normalize_rotation=normalize_rot,
            target_modules=target_modules,
            exclude_modules=exclude_modules,
        )
    else:
...
def _apply_fsdp2_compat(model, fsdp_kwargs, config, is_lora=False):
    if "is_lora" in inspect.signature(apply_fsdp2).parameters:
        return apply_fsdp2(model, fsdp_kwargs, config, is_lora=is_lora)
    return apply_fsdp2(model, fsdp_kwargs, config)
...
class FSDPSFTTrainer:
    def __init__(
        ...
        self.lora_rank = _get_lora_rank(self.config.model)
        self.oft_block_size = self.config.model.get("oft_block_size", 0)
        self.is_lora = self.config.model.get("lora_adapter_path") is not None or self.lora_rank > 0
        self.is_oft = self.oft_block_size > 0
        self.is_peft = self.is_lora or self.is_oft
        self.is_keeplora = self.is_lora and _is_keeplora_variant(self.config.model)
    ...
    def _build_model_optimizer(self):
        ...
        planned_total_steps = len(self.train_dataloader) * self.config.trainer.total_epochs
        peft_init_path = self.config.model.get("peft_adapter_path")

        if peft_init_path is not None:
            local_adapter_path = copy_to_local(src=peft_init_path, verbose=True, use_shm=self.config.model.use_shm)
            adapter_state_path = os.path.join(local_adapter_path, "adapter_model.safetensors")
            if os.path.exists(adapter_state_path):
                if self.device_mesh.get_rank() == 0:
                    print(f"Loading PEFT init state from: {adapter_state_path}")
                peft_state_dict = safe_load_file(adapter_state_path)
        elif self.is_peft and self.config.model.get("lora_adapter_path") is None and not self.is_keeplora:
            if self.device_mesh.get_rank() == 0:
                print("Rank 0: pre-calculating PEFT initialization for consistent SFT startup")
                ...
                if self.use_remove_padding or self.config.ulysses_sequence_parallel_size > 1:
                    ...
                    apply_monkey_patch(model=cpu_model, ulysses_sp_size=self.config.ulysses_sequence_parallel_size)

                if self.is_lora:
                    peft_config = _build_lora_config(self.config.model, total_training_steps=planned_total_steps)
                else:
                    peft_config = _build_oft_config(self.config.model)
                cpu_model = get_peft_model(cpu_model, peft_config)

                from peft import get_peft_model_state_dict

                peft_state_dict = get_peft_model_state_dict(cpu_model)
                del cpu_model

            objects = [peft_state_dict]
            torch.distributed.broadcast_object_list(objects, src=0)
            peft_state_dict = objects[0]
        ...
        with init_context():
            ...
            if self.config.model.get("enable_input_require_grads", False) or self.is_peft:
                self.model.enable_input_require_grads()

            keeplora_grad_map = None
            if self.is_keeplora and self.config.model.get("lora_adapter_path") is None and peft_state_dict is None:
                keeplora_grad_map = self._collect_keeplora_seed_gradients(self.model)
            ...
            if self.is_lora:
                ...
                if lora_adapter_path is not None:
                    ...
                    self.model = get_peft_model(
                        self.model,
                        _build_lora_config(self.config.model, total_training_steps=planned_total_steps),
                    )
            elif self.is_oft:
                self.model = get_peft_model(self.model, _build_oft_config(self.config.model))

            if peft_state_dict is not None:
                from peft import set_peft_model_state_dict

                set_peft_model_state_dict(self.model, peft_state_dict)

            if self.is_keeplora:
                self._apply_keeplora_init(keeplora_grad_map)
        ...
        if self.config.model.enable_gradient_checkpointing:
            self.model.gradient_checkpointing_enable(
                gradient_checkpointing_kwargs={"use_reentrant": self.config.model.get("use_reentrant", False)}
            )
        ...
        if fsdp_strategy == "fsdp":
            ...
            _apply_fsdp2_compat(self.model, fsdp_kwargs, self.config.model.fsdp_config, is_lora=self.is_peft)
        ...
        if _get_lora_variant(self.config.model) == "loraplus":
    ...
    def _get_keeplora_init_micro_batch(self):
        init_batch = next(iter(self.train_dataloader))
        if not isinstance(init_batch, TensorDict):
            first_tensor = next(v for v in init_batch.values() if hasattr(v, "shape") and len(v.shape) > 0)
            init_batch = TensorDict(init_batch, batch_size=first_tensor.shape[0])
        init_micro_batches = init_batch.split(self.config.data.micro_batch_size_per_gpu)
        return init_micro_batches[0]

    def _collect_keeplora_seed_gradients(self, model):
        if self.device_mesh.get_rank() != 0:
            return None

        batch = self._get_keeplora_init_micro_batch()
        original_device = next(model.parameters()).device
        runtime_device = torch.device(self.config.trainer.device)
        moved_for_init = original_device != runtime_device
        if moved_for_init:
        ...
        position_ids = batch["position_ids"].to(model_device)
        labels = input_ids[:, 1:].contiguous()
        loss_mask = batch["loss_mask"][:, 1:].reshape(-1).to(model_device)
        ...
        with autocast_context:
            ...
            shift_labels = labels.reshape(-1).to(shift_logits.device)
        ...
        for module_name, module in model.named_modules():
            ...
            if weight is not None and weight.grad is not None and weight.ndim == 2:
                grad_map[module_name] = weight.grad.detach().clone()
        model.zero_grad(set_to_none=True)
        if moved_for_init:
    ...
    def _apply_keeplora_init(self, keeplora_grad_map):
        freeze_keeplora_down_projection(self.model)

        if self.config.model.get("lora_adapter_path") is not None or self.config.model.get("peft_adapter_path") is not None:
            if self.device_mesh.get_rank() == 0:
                print("KeepLoRA: loaded existing adapter, skipping first-step gradient initialization")
            return

        if self.device_mesh.get_rank() == 0:
            principal_rank = resolve_keeplora_principal_rank(
                self.config.model.get("keeplora_principal_rank", 0),
                self.lora_rank,
            )
            initialized = initialize_keeplora_adapters(
                self.model,
                principal_rank=principal_rank,
                grad_map=keeplora_grad_map,
            )
            self.model.zero_grad(set_to_none=True)
            print(f"KeepLoRA: initialized {initialized} adapter modules with principal_rank={principal_rank}")
    ...
    def save_checkpoint(self, step):
        ...
        if self.device_mesh.get_rank() == 0:
            save_checkpoint_metadata(
                ...
                base_model_name_or_path=self.config.model.partial_pretrain,
                tokenizer_name_or_path=getattr(self.config.model, "tokenizer_path", None),
            )
