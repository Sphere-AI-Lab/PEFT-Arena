...
from peft_arena_verl.single_controller.ray import RayClassWithInitArgs, RayResourcePool, RayWorkerGroup
from peft_arena_verl.single_controller.ray.base import create_colocated_worker_cls
...
from peft_arena_verl.trainer.ppo.reward import compute_reward, compute_reward_async
from verl.utils.checkpoint.checkpoint_manager import find_latest_ckpt_path, should_save_ckpt_esi
from verl.utils.debug import marked_timer
from verl.utils.metric import (
    reduce_metrics,
)
from peft_arena_verl.utils.seqlen_balancing import get_seqlen_balanced_partitions, log_seqlen_unbalance
...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            from peft_arena_verl.experimental.agent_loop import AgentLoopManager
    ...
    def fit(self):
        ...
        profile_steps = OmegaConf.select(self.config, "trainer.profile_steps")

        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                do_profile = self.global_steps in profile_steps if profile_steps is not None else False
                if do_profile:
