...
class CriticConfig(BaseConfig):
    ...
    def __post_init__(self):
        ...
        if self.model_config is None and self.enable is not False:
...
class FSDPCriticModelCfg(BaseModelConfig):
    ...
    Inherits base critic settings and adds distributed-memory and PEFT options.
    ...
    Args:
        ...
        lora_adapter_path (Optional[str]): Optional pretrained LoRA adapter path.
        oft_block_size (int): Set to positive value to enable OFT.
        block_partition_method (str): OFT block partition strategy.
        block_ratio (float): OFT block ratio.
        use_doft (bool): Whether to enable DOFT behavior.
        target_modules (Union[str, List[str]]): PEFT target modules: "all-linear" or list of layer names.
        exclude_modules (Optional[Union[str, List[str]]]): Modules excluded from PEFT adaptation.
    """
    ...
    block_partition_method: str = "index"
