...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        def __collect_merged_params() -> OrderedDict:
            """
            Collect full merged params for PEFT models that don't have vLLM native support (e.g., OFT).
            Uses the merge/unmerge approach from TRL's GRPOTrainer.
            """
            ...
            if fsdp_version(self.module) > 0:
                with FSDP.summon_full_params(self.module, writeback=False):
                    ...
                    try:
                        # Get full merged state dict from base model
                        model = peft_model.base_model.model
                        for name, param in model.state_dict().items():
                            # Skip FSDP internal params and adapter-specific params
                            if any(x in name for x in ["_flat_param", "oft_", "lora_", "r_"]):
                                continue
                ...
                try:
                    model = peft_model.base_model.model
                    for name, param in model.state_dict().items():
                        if any(x in name for x in ["oft_", "lora_", "r_"]):
                            continue
                        name = name.replace(".base_layer", "")
                        merged_params[name] = param.detach().cpu()
                finally:
                    peft_model.unmerge_adapter()
            logger.info(f"vLLM load weights, loaded_params with keys: {list(merged_params.keys())}")
        ...
        with simple_timer("reshard", self.timing):
            ...
            peft_config = None
            use_merge_approach = False
            peft_model = getattr(self.module, "_fsdp_wrapped_module", self.module)
            
            if hasattr(peft_model, "peft_config"):
                peft_config = peft_model.peft_config.get("default", None)
                if peft_config is not None:
                    # Get PEFT type to determine which approach to use
                    peft_type = getattr(peft_config, "peft_type", None)
                    peft_type_str = str(peft_type) if peft_type else ""
                    
                    # Use merge approach for OFT (and other non-LoRA adapters) since vLLM doesn't support them natively
                    # LoRA uses vLLM's native adapter support
                    if "OFT" in peft_type_str or "oft" in peft_type_str.lower():
                        use_merge_approach = True
                        logger.info(f"Using merge approach for PEFT type: {peft_type_str}")
                
                if use_merge_approach:
                    params = __collect_merged_params()
                    peft_config = None  # Don't use adapter loading for merged params
                else:
    ...
    def _add_lora(self, lora_request):
        if hasattr(self.inference_engine, "add_lora"):
            return self.inference_engine.add_lora(lora_request)
        if hasattr(self.inference_engine, "llm_engine"):
            return self.inference_engine.llm_engine.add_lora(lora_request)
        raise AttributeError("Inference engine does not support add_lora")
