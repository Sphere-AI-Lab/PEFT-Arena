...
from peft_arena_verl.workers.rollout.async_server import AsyncServerBase
...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def init_engine(self):
        ...
        model_lora_rank = int(config.model.get("lora_rank", 0) or 0)
    ...
    async def generate(self, prompt_ids: List[int], sampling_params: Dict[str, Any], request_id: str) -> List[int]:
        ...
        lora_request = None
        model_lora_rank = int(self.config.model.get("lora_rank", 0) or 0)
        if model_lora_rank > 0:
            lora_ids = await self.engine.list_loras()
            if len(lora_ids) > 0:
                lora_id = next(iter(lora_ids))
