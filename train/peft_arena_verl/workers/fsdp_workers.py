...
from peft import LoraConfig, OFTConfig, OFTV3Config, TaskType, get_peft_model
...
from peft_arena_verl.single_controller.base import Worker
from peft_arena_verl.single_controller.base.decorator import Dispatch, register
...
from peft_arena_verl.workers.sharding_manager.fsdp_ulysses import FSDPUlyssesShardingManager
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        self._is_oft = self._oft_block_size > 0
    ...
    def _build_model_optimizer(
        ...
        # Pre-calculate PEFT state dict on Rank 0 for consistency
        peft_state_dict = None
        if self._is_lora or self._is_oft:
            if self.rank == 0:
                print(f"Rank 0: Pre-calculating PEFT state dict on CPU for consistency...")
                # Load model on CPU temporarily
                ...
                if self._is_lora:
                     ...
                     cpu_model = get_peft_model(cpu_model, LoraConfig(**lora_config))
                elif self._is_oft:
                    oft_config = dict(
                        task_type=TaskType.CAUSAL_LM,
                        ...
                        target_modules=convert_to_regular_types(self.config.model.target_modules),
                        exclude_modules=convert_to_regular_types(self.config.model.exclude_modules),
                        block_ratio=self.config.model.get("block_ratio", 1.0),
                        doft=self.config.model.get("use_doft", False),
                    )
                    cpu_model = get_peft_model(cpu_model, OFTV3Config(**oft_config))
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self._is_lora:
                ...
                actor_module = get_peft_model(actor_module, OFTV3Config(**oft_config))
            
            if peft_state_dict is not None:
                from peft import set_peft_model_state_dict
                set_peft_model_state_dict(actor_module, peft_state_dict)
                if self.rank == 0:
                    print("Loaded consistent PEFT weights from Rank 0.")

            # Current PEFT versions may initialize/load adapter params in fp32 even when
            # the base model has already been cast to bf16. FSDP requires uniform param dtype.
            if self._is_lora or self._is_oft:
                actor_module.to(torch_dtype)
        ...
        if fsdp_strategy == "fsdp":
            ...
            apply_fsdp2(actor_module, fsdp_kwargs, fsdp_config, is_lora=self._is_lora or self._is_oft)
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            from peft_arena_verl.workers.rollout import HFRollout
            from peft_arena_verl.workers.sharding_manager.base import BaseShardingManager
            ...
            from peft_arena_verl.workers.rollout.vllm_rollout import vLLMRollout
            from peft_arena_verl.workers.sharding_manager.fsdp_vllm import FSDPVLLMShardingManager
            ...
            # lora_kwargs = {}
            from peft_arena_verl.workers.rollout.vllm_rollout import vLLMAsyncRollout
            ...
            from peft_arena_verl.workers.rollout.sglang_rollout import SGLangRollout
            ...
            from peft_arena_verl.workers.sharding_manager.fsdp_sglang import FSDPSGLangShardingManager
    ...
    def init_model(self):
        from peft_arena_verl.workers.actor import DataParallelPPOActor
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if (self._is_lora or self._is_oft) and hasattr(getattr(self, "actor_module", self.actor_module_fsdp), "peft_config"):
            adapter_type = "lora" if self._is_lora else "oft"
            adapter_save_path = os.path.join(local_path, f"{adapter_type}_adapter")
            ...
            try:
                if fsdp_version(self.actor_module_fsdp) > 0:
                    ...
                    # Use get_peft_model_state_dict for both LoRA and OFT
                    from peft.utils.save_and_load import get_peft_model_state_dict
                    adapter_params = {}
                    with FSDP.summon_full_params(self.actor_module_fsdp, writeback=False):
                        adapter_params = get_peft_model_state_dict(peft_model)
                        adapter_params = {
                    ...
                    if dist.get_rank() == 0:
                        save_file(adapter_params, os.path.join(adapter_save_path, "adapter_model.safetensors"))
                        with open(os.path.join(adapter_save_path, "adapter_config.json"), "w", encoding="utf-8") as f:
                            json.dump(peft_config, f, ensure_ascii=False, indent=4)
            except Exception as e:
                log_with_rank(
                    f"Save {adapter_type.upper()} Adapter Error ({e})", rank=dist.get_rank(), logger=logger, log_only_rank_0=True
                )

            dist.barrier()
            log_with_rank(
                f"[rank-{self.rank}]: Saved {adapter_type.upper()} adapter to: {adapter_save_path}",
                rank=dist.get_rank(),
                logger=logger,
...
class CriticWorker(Worker, DistProfilerExtension):
    def __init__(self, config):
        ...
        self._is_oft = self.config.model.get("oft_block_size", 0) > 0
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        # Pre-calculate PEFT state dict on Rank 0
        peft_state_dict = None
        if self._is_lora or self._is_oft:
            if self.rank == 0:
                print(f"Rank 0: Pre-calculating PEFT state dict for Critic on CPU...")
                # Use AutoModelForCausalLM, assuming target modules are in the base model
                cpu_model = AutoModelForCausalLM.from_pretrained(
                    local_path,
        ...
        if self._is_lora:
            ...
            critic_module = get_peft_model(critic_module, OFTV3Config(**oft_config))
        
        if peft_state_dict is not None:
             from peft import set_peft_model_state_dict
             set_peft_model_state_dict(critic_module, peft_state_dict)
             if self.rank == 0:
                 print("Loaded consistent PEFT weights for Critic from Rank 0.")
        ...
        if config.strategy == "fsdp":
            ...
            apply_fsdp2(critic_module, fsdp_kwargs, fsdp_config, is_lora=self._is_lora or self._is_oft)
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        if config.strategy == "fsdp":
            ...
            apply_fsdp2(reward_module, fsdp_kwargs, config.model.fsdp_config, is_lora=self._is_lora or self._is_oft)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        from peft_arena_verl.utils.seqlen_balancing import get_reverse_idx, rearrange_micro_batches
